    ...
    , m_dispatcher{context.commandStack}
{
  ...
  connect(
      ...
      &QPushButton::pressed,
      this,
      &Tutorial::InspectorWidget::addBanana);

  // When the model changes, this will be called.
  con(object, &Tutorial::ProcessModel::bananasChanged, this, [=](int v) {
  ...
  object.polymorphicEntities.added.connect<&InspectorWidget::on_entityAdded>(
      this);
  object.polymorphicEntities.removing
      .connect<&InspectorWidget::on_entityRemoved>(this);
}

void InspectorWidget::on_entityAdded(const PolymorphicEntity& e)
{
  ossia::logger().info(
      "Entity {} added. PolymorphicEntity::someVirtualMethod() == {}",
      e.id().val(),
      e.someVirtualMethod());
}
