 ...
 *
 * It is *extremely* important not to save pointers to anything in the
 * commands. This is because if all commands were undone, and then redone, the
 * pointer in the command may now be invalid since the pointed-to object has
 * certainly been deleted and recreated in previous commands.
 *
 * Instead, we save paths to objects in the object tree with \ref Path.
 */
class AddBanana : public score::Command
{
  // The following macro is used to allow CMake to parseall the commands in a
  // plug-in, and to provide some metadata (e.g. the name that will be shown to
  // the user)
